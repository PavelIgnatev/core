...
export async function doAuthentication(sender: MTProtoPlainSender) {
  ...
  if (
    serverDhInner.g !== 3 ||
    serverDhInner.dhPrime.toString('hex') !==
      'c71caeb9c6b1c9048e6c522f70f13' +
  ...
  if (
    !(
      ga.greaterOrEquals(toCheckAgainst) &&
      ga.lesserOrEquals(dhPrime.minus(toCheckAgainst))
    )
  ) {
    throw new SecurityError(
  ...
  if (
    !(
      gb.greaterOrEquals(toCheckAgainst) &&
      gb.lesserOrEquals(dhPrime.minus(toCheckAgainst))
    )
  ) {
    throw new SecurityError(
