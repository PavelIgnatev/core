...
function validateCharacters(message: string) {
  ...
  if (hasLinks) {
    return {
      ...
      error:
        'Links/URLs detected\n\n' +
        'REASON: Links and URLs are strictly forbidden in ping messages\n' +
        'REQUIREMENT: Ping messages must not contain any links, URLs, or references to external resources\n' +
        'FORBIDDEN PATTERNS:\n' +
        '- Email addresses (user@domain.com)\n' +
        '- HTTP/HTTPS URLs (https://example.com)\n' +
        '- Domain names (www.example.com, example.com)\n' +
        '- IP addresses (192.168.1.1)\n' +
        '- Social media links (t.me/channel, discord.gg/server)\n' +
        '- Shortened links (bit.ly/abc, t.co/xyz)\n\n' +
        'HOW TO FIX: Remove all links and express the same meaning without referencing external resources',
    };
  }
  return { isValid: true };
}
...
export function getValidationRules(): string {
  ...
  [LINKS_AND_URLS_STRICTLY_FORBIDDEN]
    ** ZERO TOLERANCE POLICY: NO LINKS ALLOWED **
    ** ANY LINK DETECTION = AUTOMATIC REJECTION **

    FORBIDDEN PATTERNS:
    - Email addresses (user@domain.com)
    - HTTP/HTTPS URLs (https://example.com, http://example.com)
    - Domain names (www.example.com, example.com)
    - IP addresses (192.168.1.1)
    - Social media links (t.me/channel, discord.gg/server)
    - Shortened links (bit.ly/abc, t.co/xyz)

    ** CONSEQUENCE: Any detected link = message rejection **
    ** REQUIREMENT: Express all information without external references **
  [/LINKS_AND_URLS_STRICTLY_FORBIDDEN]
