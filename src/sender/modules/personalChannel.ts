...
export const personalChannel = async (
  ...
  if (!title || !about || !chatPhoto || !(chatPhoto instanceof GramJs.Photo)) {
  ...
  if (lastMessage.message && buttons.length) {
    ...
    if (
      addChannelBotControllerMessages[0].message.includes('Adding a channel') ||
      ...
      if (
        ...
        if (
          addChannelBotControllerReplyMarkup instanceof GramJs.ReplyInlineMarkup
        ) {
      ...
      if (
        addChannelBotControllerReplyMarkup2 instanceof GramJs.ReplyInlineMarkup
      ) {
      ...
      throw new Error('CONNECT_ADD_CHANNEL_BOT_CONTROLLER_MESSAGES_NOT_FOUND');
    }
    ...
    if (
      usernameBotControllerMessages[0].message.includes('Setting the time zone')
    ) {
      const usernameBotControllerMessage = await sendMessage(
        client,
        String(botControllerUserId),
    ...
    } else if (!usernameBotControllerMessages[0].message.includes('Success!')) {
  ...
  const { id: userId, accessHash, username: botUsername } = webPageBot.users[0];
