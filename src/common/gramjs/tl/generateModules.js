...
function generateSchema(input, output, reducedMethods) {
  let apiTl = fs.readFileSync(path.resolve(__dirname, input), 'utf-8');

  if (reducedMethods) {
    apiTl = stripTl(apiTl);
    const methodList = JSON.parse(
      fs.readFileSync(path.resolve(__dirname, reducedMethods), 'utf-8')
    );
...
function main() {
  ...
  generateSchema(
    './static/api.tl',
    './apiTl.js',
    !FULL_SCHEMA && './static/api.json'
  );
  generateSchema(
    './static/schema.tl',
    './schemaTl.js',
    !FULL_SCHEMA && './static/schema.json'
  );
}
